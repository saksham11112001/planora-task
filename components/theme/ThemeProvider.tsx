...
export function ThemeProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    const saved = (localStorage.getItem('upfloat-theme') as Theme) ?? 'system'
    setThemeState(saved)
    ...
    const handler = (e: MediaQueryListEvent) => {
      const current = (localStorage.getItem('upfloat-theme') as Theme) ?? 'system'
      if (current === 'system') {
  ...
  function setTheme(t: Theme) {
    localStorage.setItem('upfloat-theme', t)
    setThemeState(t)
