...
export function PortalView({ rawToken }: Props) {
  ...
  if (loading) return <PortalShell orgName="upFloat" clientName=""><LoadingState /></PortalShell>
  if (error)   return <PortalShell orgName="upFloat" clientName=""><ErrorState message={error} /></PortalShell>
  if (!data)   return null
  ...
  return (
    <div style={{
      ...
      <div style={{ borderTop: '1px solid #e2e8f0', padding: '16px 20px', textAlign: 'center' }}>
        <span style={{ fontSize: '12px', color: '#94a3b8' }}>Powered by <strong>upFloat</strong></span>
      </div>
    </div>
  )
}
