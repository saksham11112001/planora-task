        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, flexWrap: 'wrap' }}>
          ...
          {clients.length > 0 && (
            <select value={clientFilter} onChange={e => setClientFilter(e.target.value)}
              style={{
                padding: '5px 10px', borderRadius: 20, fontSize: 12, cursor: 'pointer', outline: 'none',
              ...
              <div style={{ minWidth: 0 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                  ...
                  <button onClick={e => { e.stopPropagation(); toggleSubExpand(task.id) }}
                    style={{
                      flexShrink: 0, fontSize: 10, padding: '1px 7px', borderRadius: 99, border: 'none',
                ...
                {(subtaskMap[task.id] ?? []).map((sub: any) => (
                  <div key={sub.id} style={{
                    ...
                    <button onClick={() => toggleSubDone(task.id, sub.id, sub.status, sub.title)}
                      style={{
                        width: 14, height: 14, borderRadius: '50%', border: 'none', flexShrink: 0, cursor: 'pointer',
