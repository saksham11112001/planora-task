        ...
        <div style={{ display:'flex', flexDirection:'column', flex:1, overflow:'hidden' }}>
          ...
          <div style={{ flex:1, overflowX:'auto', overflowY:'hidden', padding:'14px 20px', background:'var(--surface-subtle)', display:'flex', gap:12, alignItems:'flex-start' }}>
            {REC_BOARD_COLS.map(col => {
              let colTasks = localTasks.filter(t => (t.frequency??'').toLowerCase()===col.freq || (col.freq==='annual' && (t.frequency??'').toLowerCase()==='yearly'))
              if (boardClient) colTasks = colTasks.filter(t => t.client?.id===boardClient)
              return (
                <div key={col.freq} style={{ width:240, flexShrink:0, borderRadius:10, overflow:'hidden', maxHeight:'calc(100vh - 260px)', display:'flex', flexDirection:'column', background:'var(--border-light)', border:'1px solid var(--border)' }}>
                  ...
                  <div style={{ padding:7, display:'flex', flexDirection:'column', gap:6, overflowY:'auto', flex:1 }}>
                    {colTasks.map(task => (
                      <div key={task.id} onClick={() => setSelectedTask(selectedTask?.id===task.id ? null : task as any)}
                        ...
                        <div style={{ fontSize:12, fontWeight:500, color:'var(--text-primary)', marginBottom:5, lineHeight:1.4 }}>{task.title}</div>
                        {task.client && (
                        ...
                        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between' }}>
                          <span style={{ fontSize:10, fontWeight:600, color:PRIORITY_COLORS[task.priority]??'#94a3b8' }}>{task.priority}</span>
                          {task.next_occurrence_date && <span style={{ fontSize:10, color:'var(--text-muted)' }}>Next: {task.next_occurrence_date.slice(0,10)}</span>}
                        </div>
                      </div>
                    ))}
      ...
      {viewTab === 'List' && (
        <div>
            ...
            <div style={{ display:'flex', alignItems:'center', gap:12, flexWrap:'wrap' }}>
              ...
              {clients.length > 0 && (
                <select value={clientFilter} onChange={e => setClientFilter(e.target.value)}
                  style={{ padding:'5px 10px', borderRadius:20, fontSize:12, cursor:'pointer', outline:'none', border:clientFilter?'1px solid var(--brand)':'1px solid var(--border)', background:clientFilter?'rgba(13,148,136,0.08)':'var(--surface-subtle)', color:clientFilter?'var(--brand)':'var(--text-secondary)', fontWeight:clientFilter?600:400, fontFamily:'inherit', appearance:'none', paddingRight:20 }}>
                  <option value=''>All clients</option>
                  {clients.map(cl => <option key={cl.id} value={cl.id}>{cl.name}</option>)}
                </select>
              )}
          ...
          <div className="card-elevated overflow-hidden mb-4">
            <div style={{ display:'grid', gridTemplateColumns:'1fr 7rem 5rem 6rem 5rem 4.5rem', padding:'10px 16px', borderBottom:'1px solid var(--border)', background:'var(--surface-subtle)', fontSize:11, fontWeight:700, color:'var(--text-muted)', textTransform:'uppercase', letterSpacing:'0.05em' }}>
              <div>Task</div>
