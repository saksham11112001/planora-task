...
export function MyTasksView({ tasks: initialTasks, members, clients, currentUserId, userRole }: Props) {
    ...
    <div style={{
      ...
      {(['List', 'Board'] as const).map(t => (
        <button key={t} onClick={() => setTab(t)}
          style={{
            padding: '10px 15px', fontSize: 14, fontWeight: 500, border: 'none',
  ...
  if (tab === 'List') return (
    <>
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', background: 'var(--surface)' }}>
        ...
        <div style={{
          display: 'grid', gridTemplateColumns: '28px 22px 1fr 160px 100px 110px',
        ...
        <div style={{ flex: 1, overflowY: 'auto' }}>
          {LIST_SECS.map(sec => {
            ...
            return (
              <div key={sec.key}>
                <div style={{
                  display: 'flex', alignItems: 'center', gap: 6, padding: '13px 18px 5px',
                      ...
                      <div style={{ minWidth: 0, overflow: 'hidden' }}>
                        <div style={{
                          ...
                          {isPending && <span style={{
                            flexShrink: 0, fontSize: 11, background: '#ede9fe',
                ...
                {done.map(task => {
                  ...
                  return (
                    <div key={task.id}
                      ...
                      <CircleBtn task={task} />
                      <div style={{
                        fontSize: 13, color: 'var(--text-muted)', textDecoration: 'line-through',
  ...
  return (
    <>
      <Tabs />
      <div style={{
        flex: 1, overflowX: 'auto', overflowY: 'hidden', padding: '14px 20px',
        ...
        {BOARD_COLS.map(col => {
          ...
          return (
            <div key={col.status} style={{
              width: 268, flexShrink: 0, background: 'var(--border-light)',
                ...
                {colTasks.map(task => {
                  ...
                  return (
                    <div key={task.id}
                      onClick={() => setSelTask(selTask?.id === task.id ? null : task)}
                      style={{
                        background: 'var(--surface)', borderRadius: 8, padding: '10px 11px',
