  ...
  function TaskRow({ task }: { task: Task }) {
    ...
    return (
      ...
      <div className={cn('task-row group', selectedTask?.id === task.id && 'selected', checked.has(task.id) && 'bg-teal-50/60')}
        style={{ background: checked.has(task.id) ? 'rgba(13,148,136,0.06)' : isPending ? 'rgba(124,58,237,0.04)' : 'transparent', borderLeft: `3px solid ${selectedTask?.id === task.id ? 'var(--brand)' : 'var(--border)'}` }}>
