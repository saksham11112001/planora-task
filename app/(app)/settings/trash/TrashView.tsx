  ...
  return (
    <div>
      ...
      <p style={{ fontSize:13, color:'var(--text-muted)', marginBottom:20 }}>
        Deleted tasks are kept for 30 days. {isPaid ? 'Restore them with one click.' : 'Upgrade to restore deleted tasks.'}
      </p>
      ...
      {loading ? (
        ...
        <div style={{ border:'1px solid var(--border)', borderRadius:12, overflow:'hidden' }}>
          {/* Header */}
          <div style={{ display:'grid', gridTemplateColumns:'1fr 110px 100px',
          ...
          {tasks.map((t, i) => {
            ...
            const urgent = left <= 5
            return (
              <div key={t.id} style={{ display:'grid', gridTemplateColumns:'1fr 110px 100px',
                ...
                <div style={{ minWidth:0 }}>
                  ...
                  <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                    ...
                    <span style={{ fontSize:11, padding:'1px 6px', borderRadius:4,
                      background: PRIORITY_COLOR[t.priority]+'18' ?? '#f8fafc',
                ...
                <div>
                  ...
                  <p style={{ fontSize:11, color: urgent?'#dc2626':'var(--text-muted)', fontWeight: urgent?600:400, marginTop:2 }}>
                ...
                <div style={{ textAlign:'right' }}>
                  {isPaid && canManage ? (
                    <button onClick={() => restore(t.id)} disabled={restoring === t.id}
                      style={{ display:'inline-flex', alignItems:'center', gap:5,
                        padding:'6px 12px', borderRadius:7,
                        border:'1px solid var(--brand-border)',
                        background:'var(--brand-light)', color:'var(--brand)',
                        fontSize:12, fontWeight:600, cursor:'pointer',
                        fontFamily:'inherit', opacity: restoring===t.id ? 0.6 : 1,
                        transition:'all 0.15s' }}
                      onMouseEnter={e=>{(e.currentTarget as any).style.background='var(--brand)';(e.currentTarget as any).style.color='#fff'}}
