...
import {
  AlertCircle, Clock, Send, CheckCircle2, ChevronDown,
...
export default function InboxView({ tasks, clients, members, currentUserId, orgId, canApprove }: Props) {
  ...
  const handleDragEnd = useCallback(async (result: DropResult) => {
    ...
    try {
      if (targetColId === 'in_review') {
        await fetch(`/api/tasks/${draggableId}/approve`, {
          method: 'POST',
  ...
  }, [optimisticTasks, updateOptimistic])
        ...
        <div className="flex rounded-lg border border-slate-200 dark:border-slate-700 overflow-hidden text-sm">
          <button
            ...
            className={cn(
              ...
              view === 'board' ? 'bg-teal-600 text-white' : 'hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-600 dark:text-slate-400'
            )}
          >
          ...
          <button
            ...
            className={cn(
              'flex items-center gap-1.5 px-3 py-1.5 transition-colors',
              view === 'list' ? 'bg-teal-600 text-white' : 'hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-600 dark:text-slate-400'
            )}
          >
            <List size={14} /> List
          </button>
      ...
      {view === 'board' && (
        <DragDropContext onDragEnd={handleDragEnd}>
          <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4 pb-6">
            {COLUMNS.map(col => {
              const Icon = col.icon
              let colTasks = getColTasks(col.id)
                ...
                <div key={col.id} className={cn('flex flex-col rounded-xl border', col.border)}>
                  <div className={cn('flex items-center justify-between px-3 py-2.5 rounded-t-xl', col.headerBg)}>
                    ...
                    <span className={cn(
                      ...
                      {isDone ? totalDone : getColTasks(col.id).length}
                    </span>
                  </div>
...
// ─── Rich Task Card (shows client, assignee, due date) ────────────────────

function RichTaskCard({ task, clients, members, onClick, isDragging }: {
  task: Task
  clients: Client[]
  members: { id: string; name: string; avatar_url?: string }[]
  onClick: () => void
...
function RichListView({ tasks, clients, members, onSelectTask }: {
  tasks: Task[]
  clients: Client[]
  members: { id: string; name: string; avatar_url?: string }[]
        ...
        return (
          <div
            ...
            className={cn(
              ...
              'border border-slate-200 dark:border-slate-700',
              'hover:border-teal-400 hover:shadow-sm transition-all',
              over && 'border-l-4 border-l-red-400'
            )}
          >
