...
import {
  ...
  FileCheck, Building2, Calendar, Paperclip, ShieldCheck,
} from 'lucide-react'
import {
  COMPLIANCE_TASKS, COMPLIANCE_GROUPS,
  type ComplianceTask, type ComplianceFrequency,
} from '@/lib/data/complianceTasks'
import { loadOverrides, type AttachmentConfig } from '@/lib/compliance'
...
/* Return the effective attachments for a task (override → task defaults) */
...
export function NewClientForm() {
  ...
  async function doSubmit() {
    ...
    try {
      ...
      /* 2 — Create compliance tasks (fire-and-forget, non-blocking) */
      if (selectedCount > 0) {
        const overrides = loadOverrides()
  ...
  /* ─────────────────────────────────────────────────────────────
     STEP 2: Compliance task selection
  ───────────────────────────────────────────────────────────── */
  return (
        ...
        <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:8 }}>
          ...
          <div>
            <h2 style={{ fontSize:15, fontWeight:700, color:'#fff', margin:0 }}>
              ...
              <span style={{ color:'#5eead4' }}>{form.name}</span>
            </h2>
            <p style={{ fontSize:11, color:'rgba(255,255,255,0.5)', margin:0 }}>
              Select applicable tasks · set frequency &amp; dates · attachments are pre-configured from your templates
            </p>
          </div>
        </div>
                  ...
                  <div key={task.title}
                    ...
                    <div style={{ display:'flex', alignItems:'center', gap:10, padding:'9px 12px',
                      ...
                      <span style={{ fontSize:10, fontWeight:500, color:'var(--text-muted)',
                        ...
                        {FREQ_LABELS[sel?.frequency ?? task.frequency]}
                      </span>

                      {/* Attachments pill */}
                      {attachments.length > 0 && (
                        <span style={{ display:'flex', alignItems:'center', gap:3, fontSize:10, fontWeight:600,
                          color:'#0891b2', background:'#eff6ff', padding:'2px 7px', borderRadius:99, flexShrink:0 }}>
                      ...
                      {isSelected && (
                        <button onClick={e => {
                          ...
                          <Calendar style={{ width:10, height:10 }}/> Dates
                        </button>
                      )}
                    </div>
                        ...
                        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:10, marginBottom:10 }}>
                          <div>
                            ...
                            <select value={sel.frequency}
                              onChange={e => updateSel(task.title, 'frequency', e.target.value as ComplianceFrequency)}
                          ...
                          <div style={{ borderTop:'1px solid var(--border-light)', paddingTop:10 }}>
                            <div style={{ display:'flex', alignItems:'center', gap:6, marginBottom:7 }}>
                              ...
                              <span style={{ fontSize:10, color:'var(--text-muted)', fontStyle:'italic' }}>
                                — client must upload these to complete the task
                              </span>
                            </div>
                            ...
                            <p style={{ fontSize:10, color:'var(--text-muted)', margin:'7px 0 0', lineHeight:1.5 }}>
                              These are inherited from your CA Compliance Hub template.{' '}
