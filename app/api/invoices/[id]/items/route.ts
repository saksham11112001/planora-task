...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: item, error } = await supabase.from('invoice_items').insert({
    invoice_id:  id,
    org_id:      mb.org_id,
  ...
  const { data: allItems } = await supabase.from('invoice_items')
    .select('amount').eq('invoice_id', id)
  const subtotal    = (allItems ?? []).reduce((s, it) => s + Number(it.amount), 0)
  ...
  await supabase.from('invoices').update({ subtotal, gst_amount, total }).eq('id', id)
...
export async function DELETE(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (invoice) {
    const { data: allItems } = await supabase.from('invoice_items').select('amount').eq('invoice_id', id)
    const subtotal   = (allItems ?? []).reduce((s, it) => s + Number(it.amount), 0)
    const gst_amount = Math.round(subtotal * Number(invoice.gst_rate) / 100 * 100) / 100
    const total      = Math.round((subtotal + gst_amount - Number(invoice.discount_amount)) * 100) / 100
    await supabase.from('invoices').update({ subtotal, gst_amount, total }).eq('id', id)
